...
static int SWriteQueryPortAttributesReply(ClientPtr,
                                          xvQueryPortAttributesReply *);
static int SWriteQueryImageAttributesReply(ClientPtr,
...
ProcXvGetPortAttribute(ClientPtr client)
{
    ...
    status =
        XVCALL(diGetPortAttribute) (client, pPort, stuff->attribute, &value);
    if (status != Success) {
        client->errorValue = stuff->attribute;
        return status;
    }
...
ProcXvShmPutImage(ClientPtr client)
{
    ...
    status = XVCALL(diPutImage) (client, pDraw, pPort, pGC,
                                 ...
                                 (unsigned char *) shmdesc->addr +
                                 stuff->offset, stuff->send_event, stuff->width,
                                 stuff->height);
...
XineramaXvStopVideo(ClientPtr client)
{
    ...
    if (!
        (port =
         (PanoramiXRes *) SecurityLookupIDByType(client, stuff->port, XvXRTPort,
                                                 SecurityReadAccess)))
        return _XvBadPort;
...
XineramifyXv(void)
{
    ...
    for (i = 0; i < xvsp0->nAdaptors; i++) {
        ...
        for (j = 1; j < PanoramiXNumScreens; j++) {
            ...
            if (MatchingAdaptors[j])
                continue;       /* found it */

            /* otherwise we only look for XvImage adaptors */
            if (!(refAdapt->type & XvImageMask))
                continue;
            if (refAdapt->nImages <= 0)
                continue;
