...
#include "glyphstr.h"
#include "resource.h"
#include <nx-X11/fonts/font.h>
#include "dixfontstr.h"
#include <nx-X11/fonts/fontstruct.h>
#include "micmap.h"
#include "fb.h"
...
xxStoreColors(ColormapPtr pmap, int nColors, xColorItem *pColors)
{
	...
	expanddefs = malloc(sizeof(xColorItem)
				    * (1 <<  pScrPriv->myDepth));
...
xxInstallColormap(ColormapPtr pmap)
{
	...
	pixels = malloc(sizeof(Pixel) * (1 <<  pScrPriv->myDepth));
	colors = malloc(sizeof(xrgb) * (1 <<  pScrPriv->myDepth));
	defs = malloc(sizeof(xColorItem) * (1 << pScrPriv->myDepth));
...
static void
xxWakeupHandler (void * data, int i, pointer LastSelectMask)
{
}
